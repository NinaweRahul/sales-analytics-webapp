...
-- ---------------------------------------------------------------
-- Grain: one row per (review_id, order_id) pair -- review_id alone is
-- NOT unique in the source data (see raw.order_reviews for details).
CREATE TABLE analytics.fact_reviews (
    fact_review_id       SERIAL PRIMARY KEY,
