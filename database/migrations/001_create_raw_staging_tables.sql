...
-- ---------------------------------------------------------------
-- Note: NO foreign key from product_category_name to category_translation.
-- The real Olist data has 2 category names in products.csv that are absent
-- from the official translation file ('pc_gamer' and
-- 'portateis_cozinha_e_preparadores_de_alimentos') -- a genuine data quality
-- gap in the source, not something to paper over with a constraint that
-- would just make loading fail. The transform step (003) handles this with
-- COALESCE so untranslated categories fall back to their Portuguese name
-- instead of the whole load breaking.
...
-- ---------------------------------------------------------------
-- Note: PRIMARY KEY is (review_id, order_id), NOT review_id alone.
-- In the real data, 814 review_id values are reused across DIFFERENT
-- orders (the (review_id, order_id) pair is always unique, review_id
-- alone is not) -- another genuine source-data quirk, not a loading bug.
